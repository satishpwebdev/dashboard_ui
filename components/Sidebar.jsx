...
const Sidebar = ({ children }) => {
  ...
  return (
    <>
      <div className={`flex flex-col p-3 ${isOpen ? "w-24" : "w-56"} bg-[#0d1544]  text-white`}>
        <div className="flex items-center gap-1 py-2 px-3 my-2">
          {<TbHexagonLetterO fontSize={24} />}
          <span className="text-xl font-semibold">Dashboard</span>
        </div>
        <div className="flex-1">
          {menuItems.map((item) => (
            <LinksTag item={item}></LinksTag>
          ))}
        </div>
        <div className="btmpart">
          <div className="flex my-2 items-center px-2 rounded-md gap-2 w-full justify-between bg-[#262e5a]">
            <div className="w-12 h-12 flex items-center gap-3 ">
              ...
              <div className="flex flex-col items-start  w-full ">
                ...
                <h4 className="text-xs overflow-hidden whitespace-nowrap">Project Manager</h4>
              </div>
            </div>
            <div>
              <IoIosArrowForward></IoIosArrowForward>
